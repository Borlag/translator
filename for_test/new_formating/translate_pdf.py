...
PHRASES = [
    # --- РАЗРЕЗЫ / ВИДЫ / ВЫНОСКИ (с буквой/парой букв) ---
    # обрабатываются через regex ниже, но на случай точных совпадений:
    ("FOR ALL SECTION VIEWS SEE SHEET 2", "ВСЕ РАЗРЕЗЫ – СМ. ЛИСТ 2"),
    ...
    ("ENLARGED DETAIL Z", "УВЕЛИЧЕННАЯ ВЫНОСКА Z"),
    ("ENLARGED DETAIL", "УВЕЛИЧЕННАЯ ВЫНОСКА"),
    ("NOT TO SCALE", "БЕЗ МАСШТАБА"),
    ("LARGER DETAIL Z NOT TO SCALE", "УВЕЛИЧЕННАЯ ВЫНОСКА Z БЕЗ МАСШТАБА"),
    ("LARGER DETAIL Z", "УВЕЛИЧЕННАЯ ВЫНОСКА Z"),
    ...
    ("SECTION Y-Y BEFORE CHROMIUM PLATE", "РАЗРЕЗ Y-Y ДО ХРОМИРОВАНИЯ"),
    ("SECTION Y-Y AFTER CHROMIUM PLATE", "РАЗРЕЗ Y-Y ПОСЛЕ ХРОМИРОВАНИЯ"),
    ...
    ("SECTION C-C LOCK LINK BORE", "РАЗРЕЗ C-C — ОТВЕРСТИЕ ПОД ЗАМКОВУЮ ТЯГУ"),
    ("SECTION E-E TYPICAL 2 TRANSFER BLOCK LUGS", "РАЗРЕЗ E-E ТИПИЧНО 2 УШКА БЛОКА ПЕРЕДАЧИ"),
    ("SECTION F-F TYPICAL 2 BRAKE MANIFOLD LUGS", "РАЗРЕЗ F-F ТИПИЧНО 2 УШКА ТОРМОЗНОГО КОЛЛЕКТОРА"),
    ("SECTION H-H TOOLING LUG", "РАЗРЕЗ H-H ТЕХНОЛОГИЧЕСКОЕ УШО"),
    ...
    ("SECTION Z-Z WITHOUT BEARING", "РАЗРЕЗ Z-Z БЕЗ ПОДШИПНИКА"),
    ("SECTION Z-Z WITH BEARING", "РАЗРЕЗ Z-Z С ПОДШИПНИКОМ"),
    ...
    ("PART SECTION Z-Z", "ЧАСТИЧНЫЙ РАЗРЕЗ Z-Z"),

    # --- ПРОЧЕЕ ---
    ("LARGER VIEW AT", "УВЕЛИЧЕННЫЙ ВИД"),
    ("SECTION Z-Z", "РАЗРЕЗ Z-Z"),   # catch-all если не попало по regex
    ("DETAIL Z", "ВЫНОСКА Z"),
    ("VIEW A", "ВИД A"),
]
...
REGEX_PATTERNS = [
    ...
    (re.compile(r'\bSECTION\s+([A-Z]{1,2})-([A-Z]{1,2})\b'), r'РАЗРЕЗ \1-\2'),
    # SECTION X (без тире)
    (re.compile(r'\bSECTION\s+([A-Z]{1,2})\b'), r'РАЗРЕЗ \1'),
    # DETAIL X
    (re.compile(r'\bDETAIL\s+([A-Z]{1,2})\b'), r'ВЫНОСКА \1'),
    # VIEW X (одна буква или W, V и т.д.)
    (re.compile(r'\bVIEW\s+([A-Z]{1,2})\b'), r'ВИД \1'),
    ...
    # Замена "and" на "и" в названиях рисунков (только если не в коде)
    (re.compile(r'\band\b'), 'и'),
    # Замена "or" на "или" в контексте вариантов
...
def translate_pdf(input_path: str, output_path: str) -> None:
    ...
    for page_idx, page in enumerate(doc):
        ...
        for bbox, trans_text, font_size, is_bold in replacements:
            ...
            fitted = False
            for size_factor in [1.0, 0.9, 0.82, 0.75, 0.68, 0.60]:
                ...
                if overflow >= 0:
                    ...
                    break
            if not fitted:
                # Как минимум вставляем с минимальным размером
                page.insert_textbox(
                    bbox,
                    trans_text,
                    fontfile=FONT_FILE,
